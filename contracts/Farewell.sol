...
/// - Anyone can call `claim` after timeout; we emit an event with (email,
/// data).
...
contract Farewell is Initializable, UUPSUpgradeable, OwnableUpgradeable,
  ...
  /// @dev Encrypted recipient “string” as 32-byte limbs (each limb is an
  /// euint256) + original length.
  ...
  struct Message {
    // Encrypted recipient email (coprocessor-backed euints) + encrypted
    // skShare.
    EncryptedString recipientEmail; // encrypted recipient e-mail
    euint128 _skShare;
    // Your payload (already encrypted off-chain, e.g., tar+gpg) is fine to be
    // public bytes
  ...
  //  Solidity automatically initializes all storage variables to zero by
  //  default.
  ...
  event UserUpdated(address indexed user, uint64 checkInPeriod,
                    uint64 gracePeriod, uint64 registeredOn);
  event UserRegistered(address indexed user, uint64 checkInPeriod,
                       uint64 gracePeriod, uint64 registeredOn);
  ...
  event Claimed(address indexed user, uint256 indexed index,
  ...
  function _register(string memory name, uint64 checkInPeriod,
                     uint64 gracePeriod) internal {
  ...
  function _addMessage(externalEuint256[] calldata limbs, uint32 emailByteLen,
    ...
    require(limbs.length == (uint256(emailByteLen) + 31) / 32,
            "limb count mismatch");
  ...
  function markDeceased(address user) external {
    ...
    uint256 deadline = uint256(u.lastCheckIn) + uint256(u.checkInPeriod) +
                       uint256(u.gracePeriod);
    require(block.timestamp > deadline, "not timed out");

    // the user is considered from now on as deceased
    u.deceased = true;

    // the sender who discovered that the user was deceased has priority to
    // claim the message during the next 24h
    u.notifier = Notifier({
  ...
  function forceDeceased(address user) external {
    ...
    // the sender who marked the user as deceased has priority to claim the
    // message during the next 24h
    u.notifier = Notifier({
      notificationTime : uint64(block.timestamp),
      notifierAddress : msg.sender
    });
  ...
  function claim(address user, uint256 index) external {
    ...
    if (block.timestamp <= uint256(u.notifier.notificationTime) + 24 hours) {
      require(claimerAddress == u.notifier.notifierAddress,
    ...
    for (uint i = 0; i < m.recipientEmail.limbs.length;) {
      ...
      unchecked { ++i; } // this skips overflow checks on the increment
    }
  ...
  function retrieve(address owner, uint256 index) external view returns(
      euint128 skShare, euint256[] memory encodedRecipientEmail,
      uint32 emailByteLen, bytes memory payload, string memory publicMessage) {
